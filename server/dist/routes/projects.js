...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const pool = await (0, mysqlDatabase_1.getMysqlPool)();
        const [projectRows] = await pool.query('SELECT p.*, u.username as owner_name FROM projects p JOIN users u ON p.owner_id = u.id WHERE p.id = ?', [id]);
        if (projectRows.length === 0) {
            return res.status(404).json({ success: false, error: 'Project not found' });
        }
        const [memberRows] = await pool.query(`SELECT pm.*, u.username, u.email, u.role as user_role 
...
router.post('/', async (req, res) => {
    try {
        ...
        await pool.query('INSERT INTO projects (id, name, description, owner_id) VALUES (?, ?, ?, ?)', [id, name, description || null, ownerId]);
        // Add owner as admin member automatically
        const memberId = (0, uuid_1.v4)();
        await pool.query('INSERT INTO project_members (id, project_id, user_id, role) VALUES (?, ?, ?, ?)', [memberId, id, ownerId, 'admin']);
        res.status(201).json({
            success: true,
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const pool = await (0, mysqlDatabase_1.getMysqlPool)();
        const [result] = await pool.query('UPDATE projects SET name = ?, description = ? WHERE id = ?', [name, description, id]);
...
router.post('/:id/members', async (req, res) => {
    try {
        ...
        const [existing] = await pool.query('SELECT id FROM project_members WHERE project_id = ? AND user_id = ?', [id, userId]);
        if (existing.length > 0) {
            return res.status(400).json({ success: false, error: 'User is already a member of this project' });
        }
        const memberId = (0, uuid_1.v4)();
        await pool.query('INSERT INTO project_members (id, project_id, user_id, role) VALUES (?, ?, ?, ?)', [memberId, id, userId, role]);
...
router.put('/:projectId/members/:userId', async (req, res) => {
    try {
        ...
        const pool = await (0, mysqlDatabase_1.getMysqlPool)();
        const [result] = await pool.query('UPDATE project_members SET role = ? WHERE project_id = ? AND user_id = ?', [role, projectId, userId]);
        if (result.affectedRows === 0) {
...
router.delete('/:projectId/members/:userId', async (req, res) => {
    try {
        const { projectId, userId } = req.params;
        const pool = await (0, mysqlDatabase_1.getMysqlPool)();
        const [result] = await pool.query('DELETE FROM project_members WHERE project_id = ? AND user_id = ?', [projectId, userId]);
        if (result.affectedRows === 0) {
            return res.status(404).json({ success: false, error: 'Member not found' });
        }
