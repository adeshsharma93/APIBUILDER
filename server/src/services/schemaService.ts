...
export class SchemaService {
   ...
   */
  async getTables(connectionId: string, dbType: 'mysql' | 'sqlserver' = 'mysql'): Promise<TableInfo[]> {
    if (dbType === 'mysql') {
  ...
  private async getMysqlTables(connectionId: string): Promise<TableInfo[]> {
    ...
    for (const table of tableList) {
      ...
      const fkMap = new Map((foreignKeys as any[]).map(fk => [fk.columnName, fk]));
   ...
   */
  private async getSqlServerTables(connectionId: string): Promise<TableInfo[]> {
    const poolResult = await getUserDbPool(connectionId);
    ...
    for (const table of tableList) {
      ...
      // Process columns
      const pkColumns = pkResult.recordset.map((pk: any) => pk.columnName);
      const fkMap = new Map(fkResult.recordset.map((fk: any) => [fk.columnName, fk]));
